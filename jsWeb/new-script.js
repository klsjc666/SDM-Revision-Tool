...
const answer = document.getElementById('checks')
...
const modelImage = document.getElementById('modalImage')
...
var correctFeedbackNumbers = []

var correctAnswers = []

var chosenFeedbackNumbers = []

var mistakes = []

var hardmode = true

var baseURL = "http://sdmlabrevision.nat300.top/image/showAnswer?filename="


var xhr = new XMLHttpRequest()
xhr.open("POST","http://sdmlabrevision.nat300.top/quiz/page",false)
...
question.innerText = quizs[0].quizQuestion
questionHead.innerText = quizs[0].quizId
pickAnswers(quizs[0].quizId)
answerImage.src = baseURL + answers[0].picture
modelImage.src = baseURL + answers[0].picture
pickFeedbacks(quizs[0].quizId)
getNextCheckOptions()
...
function pickAnswers(quizId){
    xhr.open("POST","http://sdmlabrevision.nat300.top/answer/page",false)
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function(){
        if (xhr.status === 200){
            answers = JSON.parse(xhr.responseText).data
        }
    }
    xhr.send(JSON.stringify({"quizId":quizId}))
}

function pickFeedbacks(quizId){
    xhr.open("POST","http://sdmlabrevision.nat300.top/feedback/page",false)
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function(){
        if (xhr.status === 200){
...
function nextQuestion() {
    currentQuestion++
    question.innerText = quizs[currentQuestion].quizQuestion
    questionHead.innerText = quizs[currentQuestion].quizId
    currentAnswer = -1
    pickAnswers(quizs[currentQuestion].quizId)
    pickFeedbacks(quizs[currentQuestion].quizId)
    nextAnswer()
    ...
    if (currentQuestion > quizs.length-2) {
...
function pickMistakes(answerId){
    xhr.open("POST","http://sdmlabrevision.nat300.top/mistake/page",false)
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onload = function(){
        if (xhr.status === 200){
            mistakes = JSON.parse(xhr.responseText).data
        }
    }
    xhr.send(JSON.stringify({"answerId":answerId,"recent":1}))
}
...
function getNextCheckOptions(){
    ...
    correctFeedbackNumbers = (answers[currentAnswer].feedback).split(',')
    correctAnswers = correctFeedbackNumbers
    var arrayLength  = (correctFeedbackNumbers.length) + 5
    var chosenFeedbacks = new Array(arrayLength)
    chosenFeedbackNumbers = new Array(arrayLength)
    for (index in correctFeedbackNumbers){
        correctFeedbackNumbers[index] = parseInt(correctFeedbackNumbers[index],10)
        for (feedbackIndex in feedbacks){
            if (feedbacks[feedbackIndex].feedbackNumber == correctFeedbackNumbers[index]){
                while (true){
                    randomInt = getRandomInt(1,arrayLength)
                    if (chosenFeedbacks[randomInt - 1] == undefined){
                        chosenFeedbacks[randomInt - 1] = feedbacks[feedbackIndex].feedbackText
                        chosenFeedbackNumbers[randomInt - 1] = feedbacks[feedbackIndex].feedbackNumber
                        break
                    }
                }
            }
        }
        correctAnswers[index] = randomInt
    }
    var whileIndex = 0
    if (hardmode & (answers[currentAnswer].mostMistakes != "0")){
        var time = 0
        var mostMistakes = answers[currentAnswer].mostMistakes.split(",")
        while (time < 3){
            if (correctAnswers.includes(whileIndex + 1)){
                whileIndex ++
                continue
            }
            let feedbackNumber = parseInt(mostMistakes[time],10)
            chosenFeedbacks[whileIndex] = feedbacks[feedbackNumber-1].feedbackText
            chosenFeedbackNumbers[whileIndex] = feedbackNumber
            whileIndex ++
            time ++
        }
    }
    while (whileIndex < arrayLength){
        if (correctAnswers.includes(whileIndex + 1)){
            whileIndex ++
            continue
        }
        randomInt = getRandomInt(1,(feedbacks.length))
        if (!(chosenFeedbacks.includes(feedbacks[randomInt-1].feedbackText))){
            chosenFeedbacks[whileIndex] = feedbacks[randomInt-1].feedbackText
            chosenFeedbackNumbers[whileIndex] = feedbacks[randomInt-1].feedbackNumber
            whileIndex ++
        }
    }
    currentCheckOptions = chosenFeedbacks
    //currentCheckOptions = questions[currentQuestion].checkOptions
    for (let i=1; i<=currentCheckOptions.length; i++) {
        option = currentCheckOptions[i-1]
        document.getElementById('checkbox' + i).classList.remove('hide')
        document.getElementById('box' + i).checked = false
        document.getElementById('label' + i).classList.remove('correctAnswer')
        document.getElementById('label' + i).classList.remove('incorrectAnswer')
        currentCheck = document.getElementById("label" + i)
        currentCheck.innerText = option
    }
...
function nextAnswer() {
    ...
    answerImage.src = baseURL + answers[currentAnswer].picture
    modelImage.src = baseURL + answers[currentAnswer].picture
    revealButton.classList.add('hide')
    if (currentAnswer > 0) {
...
function submitMistakes(Mistakes){
    var postFeedbackNumbers = []
    for (mistake in Mistakes){
        postFeedbackNumbers.push(chosenFeedbackNumbers[Mistakes[mistake] - 1])
    }
    var xhr = new XMLHttpRequest()
    xhr.open("POST","http://sdmlabrevision.nat300.top/mistake/save",false)
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.send(JSON.stringify({"answerId":answers[currentAnswer].answerId,"feedbackNumbers":postFeedbackNumbers.sort().join(','),"recent":1}))
}
...
function markAnswer() {
    ...
    calculateMostMistakes()
}

function calculateMostMistakes(){
    pickMistakes(answers[currentAnswer].answerId)
    ...
    for (index in mistakes){
        ...
        splitArray = mistakes[index].feedbackNumbers.split(',')
        for (splitIndex in splitArray){
            countArray[(parseInt(splitArray[splitIndex],10) - 1)] += 1
        }
    }
    ...
    var highestCount = 0
    for (let i=0; i<3; i++){
        for (let count=0; count < countArray.length; count++){
            if (countArray[count] > highest){
                highest = countArray[count]
                highestIndex = count
                if (highestCount = i){
                    highestCount += 1``
                }
        ...
        console.log(highestIndex + 1)
        mostMistakes.push(highestIndex + 1)
        countArray[highestIndex] = 0
        highest = 0
        highestIndex = 0
    }
    ...
    if (highestCount == 3){
        submitMostMistakes(mostMistakes,answers[currentAnswer])
    }
}

function submitMostMistakes(mostMistakes, answer){
    var postMostMistakes = mostMistakes.join(",")
    var xhr = new XMLHttpRequest()
    xhr.open("POST","http://sdmlabrevision.nat300.top/answer/save",false)
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.send(JSON.stringify({"answerId":answer.answerId, "answerType":answer.answerType, "quizId":answer.quizId,
                            "picture":answer.picture, "feedback":answer.feedback, "approved":answer.approved,
                            "mostMistakes":postMostMistakes,"mistakesCount":mistakes.length}))
}
...
const questions = [
    ...
    },
    {
        ...
        answers: [
            { image: 'http://localhost:8080/image/showAnswer?filename=000d7b57-c3bc-4d5a-8fc5-77b2cb543dd3.png', correct: [8, 9, 12] },
            { image: 'http://localhost:8080/image/showAnswer?filename=000d7b57-c3bc-4d5a-8fc5-77b2cb543dd3.png', correct: [2, 6, 8, 10, 9, 15] },
    ...
    {
        number: '2023 Question A2',
        question: 'This is 2023 question A2',
        answers: [
            { image: '/2023/StudentA/A.png', correct: [1]},
            { image: '/2023/StudentB/A.png', correct: [1]},
            { image: '/2023/StudentC/A.png', correct: [1]},
